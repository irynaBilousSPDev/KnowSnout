...
import {
  ...
  resetTriviaPool,
  type TriviaQuizRound,
} from '@/src/services/openTriviaQuiz';
...
export default function TriviaQuizScreen() {
  const [round, setRound] = useState<TriviaQuizRound | null>(null);
  ...
  const loadRound = useCallback(async (index: number, skip: string[]) => {
    setLoading(true);
    setError(null);
    setPickedId(null);
    try {
      const next = await createTriviaQuizRound(skip);
      setRound(next);
      setRoundIndex(index);
  ...
  const onAnswer = (choiceId: string) => {
    ...
    setAvoid((prev) => [...prev, round.prompt].slice(-12));
  };
            ...
            {answered ? (
              <View className="mt-5 rounded-3xl border border-forest-100 bg-white px-5 py-5">
                ...
                <Text className="mt-3 font-body text-xs leading-5 text-forest-500">
                  {t('quiz.triviaTrustNote')}
                </Text>
                <Pressable
                  onPress={() => void Linking.openURL(round.learn.sourceUrl)}
                  className="mt-3"
                >
                  <Text className="font-body-bold text-sm text-forest-700">
                    {t('quiz.openTrivia')}
